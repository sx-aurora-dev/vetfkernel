...
REGISTER_KERNEL("DenseUpdateAssignAdd", "op_DenseUpdateAssignAdd");
REGISTER_KERNEL("DenseUpdateAssignSub", "op_DenseUpdateAssignSub");
...
extern "C" {
  ...
  int op_DenseUpdateAssignAdd(const void* arg, size_t len);
  int op_DenseUpdateAssignSub(const void* arg, size_t len);
}
...
int op_DenseUpdateAssign(const void* args, size_t len)
{
  ...
  if (p->dtype == DT_FLOAT) {
    ...
    ret = dense_update_assign<int64_t>(p->num_elements,
                                       p->dst_ptr, p->src_ptr) ;
  }
...
//
// DenseUpdateAssignAdd
//

namespace {

template <typename T>
int dense_update_assign_add(int64_t num_elements,
                    uint64_t dst_ptr, uint64_t src_ptr )
{
  T* dst = reinterpret_cast<T*>(dst_ptr);
  const T* src = reinterpret_cast<const T*>(src_ptr);
...
int op_DenseUpdateAssignAdd(const void* args, size_t len)
{
  ...
  if (p->dtype == DT_FLOAT) {
    ret = dense_update_assign_add<float> (p->num_elements,
                                          p->dst_ptr, p->src_ptr) ;
  }
  else if (p->dtype == DT_DOUBLE) {
    ret = dense_update_assign_add<double>(p->num_elements,
                                          p->dst_ptr, p->src_ptr) ;
  } else if (p->dtype == DT_INT64) {
    ret = dense_update_assign_add<int64_t>(p->num_elements,
                                           p->dst_ptr, p->src_ptr) ;
  }
...
//
// DenseUpdateAssignSub
//

namespace {

template <typename T>
int dense_update_assign_sub(int64_t num_elements,
                    uint64_t dst_ptr, uint64_t src_ptr )
{
  T* dst = reinterpret_cast<T*>(dst_ptr);
  const T* src = reinterpret_cast<const T*>(src_ptr);
...
int op_DenseUpdateAssignSub(const void* args, size_t len)
{
  ...
  if (p->dtype == DT_FLOAT) {
    ret = dense_update_assign_sub<float> (p->num_elements,
                                          p->dst_ptr, p->src_ptr) ;
  }
  else if (p->dtype == DT_DOUBLE) {
    ret = dense_update_assign_sub<double>(p->num_elements,
                                          p->dst_ptr, p->src_ptr) ;
  } else if (p->dtype == DT_INT64) {
    ret = dense_update_assign_sub<int64_t>(p->num_elements,
                                           p->dst_ptr, p->src_ptr) ;
  }
