...
int op_Conv2d_Backprop_Input(VEOpArgs const& args)
{
...
#ifdef _DEBUG
  printf("op_Conv2d_Backprop_Input()\n");
#endif
  ...
  const vml::Tensor* out_bp = args.arg<vml::Tensor>(0);
  const vml::Tensor* filter = args.arg<vml::Tensor>(1);
  const vml::Tensor* in_bp  = args.arg<vml::Tensor>(2);
  ...
  ret = vml::conv2d_backprop_input(*out_bp, *filter, *in_bp, v_params);
...
DEFINE_KERNEL(Conv2DBackpropInput, op_Conv2d_Backprop_Input);
