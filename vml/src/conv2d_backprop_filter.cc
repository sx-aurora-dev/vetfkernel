				...
				std::vector<int> params) // stride[W,H],dilation[W,H],padding[W,H],data_format
{
    ...
    fprintf(stderr, "conv2d_backprop_filter: filter_bp  (N,C,H,W) = (%d,%d,%d,%d)\n",
            filter_bp.dim_size[0], filter_bp.dim_size[1],
	    filter_bp.dim_size[2], filter_bp.dim_size[3]);
    ...
    void *pGradFilter = filter_bp.ptr<void*>();
    ...
    ParamGradFilter.height     = filter_bp.dim_size[2];
    ParamGradFilter.width      = filter_bp.dim_size[3];
